...
import bot
from bridge import ResponseQueue, bridge_send
...
class Whitelist(commands.Cog):
    ...
    async def handle_whitelist(self, response_queue: ResponseQueue) -> tuple[bool, str]:
    ...
    async def whitelist_add(self, interaction: discord.Interaction, server: app_commands.Choice[str], user: str):
        ...
        result = await self.handle_whitelist(self.bot.response_queue)
        if result[0]:
